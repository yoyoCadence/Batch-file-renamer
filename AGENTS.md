...
- **Storage / data model:** No backend or database. Working state (source/template file references, directory handles, preview rows, scope filter, excluded rows) is in browser memory only and is lost on reload — file handles cannot be persisted. Three separate `localStorage` keys hold what does survive: `batch-file-renamer.settings` (appearance/language), `batch-file-renamer.presets` (named rule sets), and `batch-file-renamer.session` (last-used rules, value list, mode). Anything read back from storage is untrusted and must go through `normalizeRules` / `normalizeSettings`. CSV export/import remains the handoff format.
- **Test coverage:** Two suites. `npm run test` runs `tests/*.test.mjs` under the Node test runner: the rule engine and validation (`rules.test.mjs`), settings/i18n/persistence (`settings.test.mjs`), and static wiring plus four-language translation-key completeness (`static.test.mjs`). `npm run test:e2e` runs Playwright against the shipped files in Chromium; it is the only layer that catches whole-app failures such as a module-scope temporal-dead-zone error. `npm run build` aliases `npm run test`.
- **Deployment / cache notes:** Static PWA under `pwa/`. Serve from localhost or HTTPS for service worker and File System Access API support. **The fetch handler is cache-first with no revalidation, so `CACHE_NAME` in `pwa/service-worker.js` must be bumped in any change that touches a cached shell file** (`index.html`, `assets/*`) — otherwise returning users keep serving the old build and never see the change. Currently `batch-file-renamer-v7`; `tests/static.test.mjs` asserts the value so the bump is not silently forgotten.
